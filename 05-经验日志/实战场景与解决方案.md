...
| [场景13](#场景13-ai写作衔接断层) | AI 写小说每节断层跳跃 | Prompt 接续锚点 | ⭐⭐ |
| [场景14](#场景14-db设计树形改平铺) | 自引用表查询复杂、孤儿数据 | 表设计 + 数据迁移 | ⭐⭐⭐ |
...
| 步骤 | 做法 | 效果 |
|------|------|------|
| 1. RAG 检索增强 | 每次回答前先检索相关文档，见 `RAG检索增强生成` | ✅ 减少 70% 幻觉 |
...
**涉及知识**：
- `LangGraph状态机.md` — 状态机控制流程
- `规划与推理.md` — ReAct 循环 + 反思
- `工具调用.md` — 工具返回值设计
...
**涉及知识**：
- `工具调用.md` — Function Calling 最佳实践
- `Prompt工程.md` — 工具描述撰写技巧
...
**涉及知识**：
- `部署与运维.md` — FastAPI + SSE
- `LangChain实战.md` — stream/astream
...
**涉及知识**：
- `Multi-Agent多智能体.md` — 协作模式与角色设计
- `LangGraph状态机.md` — 流程控制
...
| 13 | AI写小说每节断层 | 上下文没给接续锚点 | Prompt 末尾加 `⚠️ 上一节结尾（请从这里接续）` | 06 |
| 14 | 模型不听格式约束 | Prompt 太长稀释了关键指令 | 缩到 15 行 + 加入 Few-shot 示例 | 06 |
| 15 | LLM 返回旧版本数据 | INSERT 追加而非 UPDATE | 改用 UPDATE OR INSERT，每节只留一行 | 03 |
| 16 | 章节排序混乱 | 依赖字母序/ID序 | 加 `sort_order` 整数列显式排序 | 03 |

## 场景 13：AI 写作衔接断层

**典型现象**：AI 写第 3 节时，内容从第 1 节的结尾开始写，跳过了第 2 节。

**根因**：发了 16 节完整上下文（~110K 字），模型注意力分散，找不到该从哪儿接。
...
**涉及篇目**：[Prompt工程](../02-Areas/AI与Agent/Prompt工程.md)，经验笔记：[AI小说家](../04-Archives/AI小说家.md)

## 场景 14：DB 设计 — 树形改平铺

**典型现象**：用 `parent_id` 自引用表存储大纲层级，查询"当前卷所有节"需要递归 SQL，删除节点时孤儿数据难清理。

**根因**：大纲只有固定三层（卷→章→节），不需要无限深度，自引用表过度设计。

**最快修复**：改为平铺三列 `volume_title + chapter_title + section_order`，查询一条 `ORDER BY sort_order` 搞定。

**涉及篇目**：[记忆系统](../02-Areas/AI与Agent/记忆系统.md)，经验笔记：[AI小说家](../04-Archives/AI小说家.md)

## 参考来源

- 相关笔记: `02-Areas/AI与Agent/` 和 `05-经验日志/` 目录下的对应条目
- 本手册: `Agent核心概念.md` 到 `学习路径与转型指南.md`
