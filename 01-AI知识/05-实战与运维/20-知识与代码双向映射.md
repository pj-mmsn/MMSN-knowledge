...
| `05-RAG检索增强生成.md` | 文档→向量→检索→生成 + 进阶RAG | `agent-project-py/src/agent_rag.py` (MiniRAG + HybridRAG) | — |
...
| `src/agent_rag.py` | `MiniRAG` | 基础 RAG：向量检索→LLM生成 | `05-RAG检索增强生成.md`（基础） |
| `src/agent_rag.py` | `HybridRAG` | ⭐ 进阶 RAG：混合检索+Query重写 | `05-RAG检索增强生成.md`（进阶） |
| `src/agent_rag.py` | `MixedRetriever` | 向量+BM25+RRF混合检索器 | `05-RAG检索增强生成.md`（混合检索） |
| `src/agent_rag.py` | `rewrite_query()` | Query 重写：自然语言→搜索关键词 | `05-RAG检索增强生成.md`（Query转换） |
