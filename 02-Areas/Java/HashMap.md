...
### 为什么一条 `hash & oldCap` 就够了

```
旧容量 n=16 (0b10000)，定位用低 4 位：hash & 15 (0b01111)
新容量 2n=32，定位用低 5 位：hash & 31 (0b11111)
                      ↑
                多出来的是第5位，权重 = 16 = 旧容量

key1: hash = 0b...0 0101  →  第5位=0 → 新槽位 = 0 0101 = 5 (原地不动)
key2: hash = 0b...1 0101  →  第5位=1 → 新槽位 = 1 0101 = 21 (5+16)

hash & oldCap = hash & 16 = hash & 0b10000
  → 结果 0  → 第5位=0 → 留原地
  → 结果 16 → 第5位=1 → 原下标+旧容量
```

**本质**：扩容只多了一位参与定位，这一位的权重正好是旧容量。所以不需要重新 `hash % 新容量`，一条位与就搞定了——既省了除法，又把元素均匀分成两拨。

> 📌 位运算原理详见：[位运算基础与实战](位运算基础与实战.md)
